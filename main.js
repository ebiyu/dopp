...
const freqSpan = document.getElementById('freqSpan');
...
let circles = [];
let caston = '0';
let hagen = { x: width / 4, y: height / 2, direction: 'right' };
...
function updatePosition() {
    ...
    if (mouseMode == 'hagen') {
        ...
        switch (hagen.direction) {
            case 'left':
                if (dx > 0)
                    hagen.direction = 'right';
                break;
            case 'right':
                if (dx < 0)
                    hagen.direction = 'left';
                break;
        }
    }
    {
...
function draw() {
    ...
    ctx.drawImage(icaston, 0, 0, 800, 800, observer.x - castonSize / 2, observer.y - castonSize / 2, castonSize, castonSize);
